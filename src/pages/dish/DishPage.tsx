...
const DishPage= () => {
  ...
  const [stage, setStage] = useState(0); // 상태를 관리하는 변수 (0: 사진 업로드 전, 1: 설거지 시작, 2: 설거지 끝내기)

  const { scene: roomieScene } = useGLTF("./roomie1.glb");
  const { scene: bubbleScene } = useGLTF("./Roomie_bubble.glb");
  ...
  const renderText = () => {
    switch(stage) {
      case 0: 
        return (
          <>
            <SubTitle>설거지를 시작할까요?<br/></SubTitle>
            <SubTitle>설거지를 시작하기 전의 상태를<br/></SubTitle>
            <SubTitle>업로드해주세요<br/></SubTitle>
          </>
        );
      case 1:
        return (
          <>
            <SubTitle>설거지를 시작하려면<br/></SubTitle>
            <SubTitle>'시작하기' 버튼을 눌러주세요<br/></SubTitle>
          </>
        );
      case 2:
        return (
          <>
            <SubTitle>설거지를 끝내려면<br/></SubTitle>
            <SubTitle>'끝내기' 버튼을 눌러주세요.<br/></SubTitle>
          </>
        );
      case 3:
        return (
          <>
            <SubTitle>설거지를 아주 잘하셨네요!<br/></SubTitle>
            <SubTitle>점수는 nn점 입니다<br/></SubTitle> {/*api 추가*/}
          </>
        );
      default:
        return null;
